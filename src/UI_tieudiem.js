      ...
      cols.forEach((v, idx) => {
        ...
        if (v === "LUC_COLUMN") {
          ...
          td.onclick = (e) => {
            ...
            input.onkeydown = (ev) => {
              if (ev.key === "Enter") {
                window.postMessage({ type: "UPDATE_SOLAN_CHODOI", id: item.id, value: parseInt(input.value) }, "*");
                input.remove();
